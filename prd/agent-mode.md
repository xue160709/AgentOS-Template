...
| P1 | AppShell Agent 设置模态 | Agent Mode 开启后右上角入口切换为设置按钮，提供常规、卡片顺序和 Skills |
...
type AgentSettingsPanelId = 'general' | 'card-order' | 'skills'
```
...
- Agent 设置模态左侧设置项包含“卡片顺序”、“Skills”和“常规”；模态本身不提供统一 footer。
...
- 保存 Skills 面板时只写入 `skillModelOverrides`，必须保留已保存的 `enabled`、`todoEnabled`、`user` 和 `identity`。
